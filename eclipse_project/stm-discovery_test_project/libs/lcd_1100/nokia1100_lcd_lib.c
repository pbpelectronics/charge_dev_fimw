...
void nlcd_Init(void) {
	...
	Delay(0x1f); // выжидем не менее 5мс для установки генератора(менее 5 мс может неработать)
...
void nlcd_Clear(void) {
	...
	nlcd_xcurr = 0;
	nlcd_ycurr = 0; // Устанавливаем в 0 текущие координаты в видеобуфере
...
void nlcd_Putc(uint8_t c) {
	if (c > 127)
		c = c - 64; // Переносим символы кирилицы в кодировке CP1251 в начало второй
...
void nlcd_Print(char * message) {
	while (*message)
		nlcd_Putc(*message++); // Конец строки обозначен нулем
}
...
void nlcd_PrintWide(uint8_t * message) {
	while (*message)
		nlcd_PutcWide(*message++); // Конец строки обозначен нулем
}
...
void nlcd_Pict(uint8_t x, uint8_t y, uint8_t * picture) {
	uint8_t pict_width = picture[0]; // ширина спрайта в пикселах
	uint8_t pict_height = picture[1]; // высота спрайта в пикселах
	uint8_t pict_height_bank = pict_height / 8
			+ ((pict_height % 8) > 0 ? 1 : 0); // высота спрайта в банках
	...
	for (uint8_t i = 0; i < pict_height_bank; i++) { // проход построчно (по банкам)
			...
			for (uint8_t j = 0; j < pict_width; j++) {
				if ((x + j) < NLCD_X_RES)
					nlcd_SendByte(DATA_LCD_MODE,
							picture[adr]); // не выводить картинку за пределами экрана
				adr++;
			}
		}
	}
