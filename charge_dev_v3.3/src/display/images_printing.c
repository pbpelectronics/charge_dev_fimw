...
void printAdditionalMenuDigits(double val) {
	...
	if (integerPart >= 10) {
		...
		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[integerPartDigs[1]]);
		xPosition += ADDITIONAL_MENU_DIGITS_WIDTH;

		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[integerPartDigs[0]]);
		xPosition += ADDITIONAL_MENU_DIGITS_WIDTH;
...
//		xPosition += WIDTH_POINT_IMAGE;
		...
		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[fractionalPartDigs]);
	} else {
		char integerPartDigs = integerPart;
		...
		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[integerPartDigs]);
		xPosition += ADDITIONAL_MENU_DIGITS_WIDTH;
...
//		xPosition += WIDTH_POINT_IMAGE;
		...
		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[fractionalPartDigs[1]]);
		xPosition += ADDITIONAL_MENU_DIGITS_WIDTH;

		nlcd_Pict(xPosition, ADDITIONAL_MENU_DIGITS_TOP_MARGIN,
				additionalMenuDigitals[fractionalPartDigs[0]]);
	}
}
...
void printAdditionalMenuAnimation(int value) {
	...
	for (animationIdx = value; animationIdx < 6; animationIdx++) {
		nlcd_Pict(
				ADDITIONAL_MENU_RIGTH_ANIMATION_LEFT_MARGIN
						+ animationIdx * ADDITIONAL_MENU_ANIMATION_WIDTH,
				ADDITIONAL_MENU_RIGHT_ANIMATION_TOP_MARGIN,
				additionalMenuRightAnimation[0]);
		nlcd_Pict(
				ADDITIONAL_MENU_LEFT_ANIMATION_LEFT_MARGIN
						- animationIdx * ADDITIONAL_MENU_ANIMATION_WIDTH,
				ADDITIONAL_MENU_LEFT_ANIMATION_TOP_MARGIN,
...
void printMainImageTemplate() {
	nlcd_Pict(POSITION_MAIN_IMAGE_FIRST_PART_X,
			POSITION_MAIN_IMAGE_FIRST_PART_Y, main_image_first_part);
	nlcd_Pict(POSITION_MAIN_IMAGE_THIRD_PART_X,
			POSITION_MAIN_IMAGE_THIRD_PART_Y, main_image_third_part);
}
...
void printChangeAmperageImage(int imageIdx) {
	...
	for (idx = 1; idx <= imageIdx; idx++) {
		nlcd_Pict(POWER_IMAGE_LEFT_INDENT + POWER_IMAGE_WIDTH * idx,
				POWER_IMAGE_Y_POSITION, power_images[idx]);
	}
	for (idx = imageIdx + 1; idx < 11; idx++) {
		nlcd_Pict(POWER_IMAGE_LEFT_INDENT + POWER_IMAGE_WIDTH * idx,
				POWER_IMAGE_Y_POSITION, power_images[0]);
	}
}
...
void printInitChangeAmperageMenu() {
	...
	nlcd_Pict(MENU_AMPERAGE_SYMBOL_LEFT_INDENT, MENU_AMPERAGE_SYMBOL_TOP_INDENT,
			menuAmperageSymbol);
}
